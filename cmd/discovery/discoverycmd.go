...
import (
	"database/sql"
	"github.com/unicornultrafoundation/network-stats/pkg/discovery"
	"os"
...
var (
	discoveryCommand = &cli.Command{
		Name:   "discovery",
		Usage:  "Discover network nodes",
		Action: discoveryNodes,
		Flags: []cli.Flag{
			autovacuumFlag,
			bootnodesFlag,
			busyTimeoutFlag,
...
func discoveryNodes(ctx *cli.Context) error {
	...
	for {
		updatedSet := discover.CrawlRound(inputSet, db, geoipDB)
		if nodesFile != "" {
