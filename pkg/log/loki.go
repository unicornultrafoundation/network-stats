...
func New(ctx context.Context, cfg LokiConfig) ZapLoki {
	c := &http.Client{}

	cfg.Url = strings.TrimSuffix(cfg.Url, "/")
	cfg.Url = fmt.Sprintf("%s/loki/api/v1/push", cfg.Url)

	fmt.Println("Config url", cfg.Url)
	pusher := &lokiPusher{
		config:  &cfg,
		ctx:     ctx,
		client:  c,
...
func (lp *lokiPusher) WithCreateLogger(cfg zap.Config) (*zap.Logger, error) {
	...
	fmt.Println("Full sink key", fullSinkKey)
...
func (lp *lokiPusher) send(batch []logEntry) error {
	fmt.Println("Send batch log entry")
